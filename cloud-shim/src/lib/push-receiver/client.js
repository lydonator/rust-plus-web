...
module.exports = class Client extends EventEmitter {
  ...
  _onMessage({ tag, object }) {
    console.log(`[PushReceiver] _onMessage tag: ${tag}`);
  ...
  _onDataMessage(object) {
    console.log('[PushReceiver] _onDataMessage called', JSON.stringify(object, null, 2));

    if (this._persistentIds.includes(object.persistentId)) {
      console.log('[PushReceiver] Duplicate persistentId, ignoring:', object.persistentId);
      return;
    }

    // Check if crypto-key is present in appData
    let hasCryptoKey = false;
    if (object.appData && Array.isArray(object.appData)) {
      hasCryptoKey = object.appData.some(item => item.key === 'crypto-key');
    }

    // if crypto keys not provided, notification is probably not encrypted, so return message as is
    if (!hasCryptoKey) {
      console.log('[PushReceiver] No crypto-key found, emitting ON_DATA_RECEIVED');
      this._persistentIds.push(object.persistentId);
      this.emit('ON_DATA_RECEIVED', object);
      return;
    }

    console.log('[PushReceiver] Crypto-key found, attempting decryption...');
